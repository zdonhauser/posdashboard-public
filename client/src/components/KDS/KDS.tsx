...
const KDS: React.FC<KDSProps> = ({ mode = "kitchen" }) => {
  ...
  useEffect(() => {
    ...
    const socket = io(endpoint, {
      ...
      timeout: 20000,
      // you can tighten ping intervals if you want faster detection:
      // pingInterval: 10000,
      // pingTimeout: 5000,
    });
    ...
    const markDown = () => {
      setConnected(false);
      if (lossToast == null) {
        lossToast = toast.warn("Realtime link lost… if issue persists, please restart the application", {
          autoClose: false,
        });
      }
    };
  ...
  const fetchKDSOrders = async () => {
    ...
    // ★ FIXED: now mirrors the other helpers
  ...
  return (
    <div className="kds-container">
      {/*<div
        className={`status-dot ${connected ? "ok" : ""}`}
        title={connected ? "Realtime OK" : "Disconnected"}
      />*/}
