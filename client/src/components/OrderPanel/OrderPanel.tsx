    ...
    transactions?.forEach((transaction, index) => {
      ...
      priceArray.push(
        ...
        >
          ...
          {transaction.kind === "refund" || transaction.kind === "change" ? `(${formatCurrency(transaction.amount)})` : formatCurrency(transaction.amount)}
        </b>
      );
      ...
      if (orderId) {
        if (isCanceled) {
          ...
          if (fulfilled_quantity) {
            orderArray.push(
              <p key={"orderlinef-" + i} className={`listItem ${mode === "front" ? "fulfilled" : ""}`}>
            ...
            orderArray.push(
              <p key={"linepricef-" + i} className="listItemPrice fulfilled">
                {items[i].fulfillable_quantity == 0 ? formatCurrency(items[i].quantity * items[i].price) : "" + "\n"}
              </p>
            );
          }
          if (refundedQty) {
            orderArray.push(
              <p key={"orderlinef-" + i} className={`listItem ${mode === "front" ? "refunded" : ""}`}>
